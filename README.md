...
提供总览、每日 Token / 美元金额合计、每日模型明细、单模型历史查询与趋势图，直接只读访问现有统计库。
网页在 `ai-monitor` 进程内运行，退出、终端断开或强制结束时同步关闭，无独立子进程。
自定义端口和数据库配置见 [网页统计说明](crates/herdr-usage/README.md#本地网页统计)。
