...
| 按键 | 操作 |
| --- | --- |
| `r` | 刷新所有额度和 AI 线路；重复请求合并，遵守服务端退避时间 |
...
依赖系统 `curl`（Debian：`sudo apt install curl`）。左栏系统资源和云端额度之间显示四条独立线路；高度不足以保留系统区 12 行和额度区 8 行时，线路内容进入额度滚动列表顶部。内部宽度不足 36 列时分两行，不足 24 列时逐项显示。

| 线路 | HTTPS 探测入口 |
| --- | --- |
| Codex | `https://chatgpt.com/backend-api/codex/responses` |
| OpenCode Go | `https://opencode.ai/zen/go/v1/chat/completions` |
| SuperGrok | `https://cli-chat-proxy.grok.com/v1/chat/completions` |
| Antigravity | `https://daily-cloudcode-pa.googleapis.com/v1internal:streamGenerateContent` |

每条线路启动立即探测，默认每 10 秒一次，超时 5 秒，独立运行。通过 Rust Command 直接启动 curl，禁用 `.curlrc`，发送无凭据 HEAD，不发送提示词、不调用模型、不消耗模型 token；不跟随重定向，保留证书验证。每次建立新连接，继承监控进程的代理环境变量和系统路由。Codex 选择 ChatGPT 订阅入口，但探测不读取或发送订阅凭据。

- **建连**：开始到 TLS 握手完成，含 DNS、TCP、TLS。
- **响应**：开始到收到首个 HTTP 响应字节，含建连。
- **连通率**：单调时钟最近 5 分钟内收到 HTTP 响应的次数 ÷ 已完成探测次数；不足 3 次显示“采样中”。暂停和未执行的请求不计为失败。

401、403、404、405、429 和 5xx 均表示入口网络可达，不能证明模型可用；403、429、5xx 显示状态提示，429 遵守 Retry-After（秒数或 HTTP 日期）。DNS、连接、TLS、超时失败计入连通率，当前延迟清空为 `—`。失败红色；响应超过 1.5 秒、连通率低于 99% 或有服务提示时黄色；其余绿色，颜色仅表示网络提示。

`r`、鼠标刷新同时触发线路探测，重复请求合并，不重叠，最短间隔 5 秒且遵守服务端退避。缺少 curl 时显示原因，其他功能继续运行。样本仅在内存保存，退出时终止并回收探测子进程；Linux 父进程异常退出时也会终止 curl。

这里测量监控进程到 HTTPS 服务入口的通信质量，不测模型首字时间、token 速度或长连接稳定性。`100% − 连通率` 是探测失败率，不是数据包丢包率；TCP 重传会掩盖实际丢包，DNS/TLS 错误也可能导致探测失败。
...
network_interval_seconds = 10 # 5–300 秒
