...
- **左栏**：上半是系统资源，下半是 Codex 常规 GPT、GPT-5.3-Codex-Spark、AGY、AGY2、
  OpenCode Go、SuperGrok 和 OpenRouter 的**云端剩余配额**。额度列表在系统数字下方独立滚动，
  不会把系统指标顶出画面。
- **右栏**：本机实际烧掉的 token —— 按模型排行（输入/缓存命中率/输出/思考/金额）、
  当日、本周与当月的柱状图、三个区间合计和历史总计。
...
`97.47%`）。两栏共用同一个滚动位置，窗格不够高时上下键同时翻动额度列表和 token 列表。
...
版本号以 `Cargo.toml` 的 `version` 为唯一来源（当前 `v0.4.0`）：页脚、HTTP User-Agent 和本文件的最新更新小节都由它派生，改动版本只需改 `Cargo.toml` 一处；`the_readme_changelog_matches_the_crate_version` 会在文档与它不一致时失败。
