...
use ratatui::{
    ...
    symbols::Marker,
    text::{Line, Span},
    widgets::{Paragraph, canvas::{Canvas, Line as CanvasLine}},
};
...
pub(super) fn draw(
    ...
    let Series::Money(costs) = &chart.series else { return };
    ...
    let Some(geometry) = Geometry::new(area.width, origin, costs.len()) else {
        frame.render_widget(Paragraph::new(vec![
            Line::styled(truncate(&title, area.width as usize), Style::default().fg(CYAN)),
            Line::styled(truncate("请拉宽查看完整月份", area.width as usize), Style::default().fg(MUTED)),
        ]), area);
        return;
    };
    ...
    let plot = Rect::new(area.x + origin as u16, area.y + 1,
                         geometry.plot as u16, rows as u16 + 1);
        ...
        .paint(|context| {
            ...
            for day in 0..shown {
                if let Some(value) = daily[day] {
                    context.print(ticks[day].0 as f64, value,
                        Line::styled("●", Style::default().fg(bar_color(value / max))));
                }
            }
        });
...
mod tests {
    ...
    fn render(costs: &[f64], width: u16, through_day: usize) -> Buffer {
        let chart = Chart {
            label: "本月金额", series: Series::Money(costs),
            span_seconds: 6 * 3600, tick_buckets: 4, first_tick: 1,
        };
        let mut terminal = Terminal::new(TestBackend::new(width, 14)).unwrap();
        terminal.draw(|frame| draw(frame, frame.area(), &chart, 8, through_day)).unwrap();
    ...
    fn four_six_hour_costs_become_one_daily_amount_without_changing_the_sum() {
        ...
        assert_eq!(totals(&costs).into_iter().flatten().sum::<f64>(), costs.iter().sum());
    ...
    fn one_point_per_elapsed_day_and_no_future_zero_line() {
        for days in [28usize, 29, 30, 31] {
            ...
            for day in 0..12 { costs[day * 4] = (day % 4 * 25) as f64; }
            for width in [(8 + days * 2) as u16, 100, 133, 180] {
                let buffer = render(&costs, width, 12);
                let points: Vec<_> = buffer.content.iter().enumerate()
                ...
                for day in 0..12 {
                    assert_eq!(points.iter().filter(|(x, _)| *x == 8 + geometry.centre(day * 4)).count(), 1);
                }
                ...
                for x in 8 + geometry.centre(12 * 4)..width as usize {
                    for y in 1..=10 { assert!(buffer[(x as u16, y)].symbol().trim().is_empty()); }
                }
            }
        }
    }
    ...
    fn actual_curve_connects_daily_points_and_does_not_draw_money_bars() {
        ...
        costs[0] = 10.; costs[4] = 90.; costs[8] = 20.;
        let buffer = render(&costs, 140, 3);
        assert_eq!(buffer.content.iter().filter(|cell| cell.symbol() == "●").count(), 3);
        assert!(buffer.content.iter().any(|cell| cell.symbol().chars().any(|ch| ('\u{2801}'..='\u{28ff}').contains(&ch))));
        assert!(!buffer.content.iter().any(|cell| cell.symbol().chars().any(|ch| "█▌▐▁▂▃▄▅▆▇".contains(ch))));
    }

    #[test]
    fn invalid_day_breaks_the_curve_instead_of_fabricating_a_point() {
        let mut costs = vec![0.; 120];
        costs[0] = 10.; costs[4] = f64::NAN; costs[8] = 20.;
        let buffer = render(&costs, 140, 3);
        assert_eq!(buffer.content.iter().filter(|cell| cell.symbol() == "●").count(), 2);
        assert!(!buffer.content.iter().any(|cell| cell.symbol().chars().any(|ch| ('\u{2801}'..='\u{28ff}').contains(&ch))));
    }
}
