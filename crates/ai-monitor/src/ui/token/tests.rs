...
fn visible_halves(buffer: &Buffer, width: u16, row: u16) -> Vec<Option<Color>> {
    ...
    for x in 8..width {
        ...
        match cell.symbol() {
            ...
            "▌" => result.extend([Some(cell.fg), (cell.bg != Color::Reset).then_some(cell.bg)]),
    ...
    for count in [96usize, 112, 116, 120, 124] {
        ...
        for plot in count / 2..=420 {
            ...
            assert!(
                ...
                "internal gap: count {count}, plot {plot}"
            );
            ...
            assert_eq!(
                widths.len(),
                count,
                "missing/combined bucket: count {count}, plot {plot}"
            );
            assert!(
                widths.iter().all(|width| *width == widths[0]),
                "unequal widths: {widths:?}"
            );
...
fn production_amount_chart_is_a_dotted_line_with_the_same_monthly_time_axis() {
    ...
    for width in [68u16, 100, 128, 139, 400] {
        ...
        assert_eq!(
            tokens[11].to_string().split_once('└').unwrap().1,
