    ...
    UsageStats {
        month_models: (0..10)
            .map(|index| ModelUsage {
                ...
                output: 0,
                reasoning: 0,
                cost: if index == 3 { None } else { Some(12.34 + index as f64) },
            })
            .collect(),
        hours: Bucketed { buckets: vec![1_000_000; 96], costs: vec![0.01; 96] },
    ...
    for days in [28, 29, 30, 31] {
        ...
        for chart in &charts {
            for width in 10..=240 {
                ...
                assert_eq!(ticks.last().unwrap().1, (chart.first_tick as usize + chart.axis_units - 1).to_string());
                assert_eq!(ticks, tick_positions(chart, axis.width), "scale changed after snapping");
                for pair in ticks.windows(2) {
                    ...
                    assert_eq!(pair[1].0 - pair[0].0, axis.pitch, "{days} days / {width} cells");
...
fn narrow_september_axes_use_five_day_steps_not_rounded_sampling() {
    ...
    for chart in &charts[1..] {
        let labels: Vec<_> = tick_positions(chart, 44).into_iter().map(|(_, label)| label).collect();
...
fn labels_and_daily_stems_share_the_exact_same_calendar_grid() {
    ...
    for width in 10..=180 {
        ...
        let columns: Vec<_> = heights.iter().enumerate().filter_map(|(x, h)| (*h > 0).then_some(x)).collect();
    ...
    for days in [28, 29, 30, 31] {
        ...
        for chart in &charts[1..] {
            for width in [16, 32, 40, 49, 60, 95, 120, 160, 220] {
                ...
                terminal.draw(|frame| super::super::draw_stem_chart(frame, frame.area(), chart, 12)).unwrap();
                let buffer = terminal.backend().buffer();
                let row = row_text(buffer, buffer.area, 11);
                let labels: Vec<usize> = row.split_whitespace().map(|s| s.parse().unwrap()).collect();
                assert_eq!(labels.last(), Some(&days), "{width}: {row}");
                let gaps: Vec<_> = labels.windows(2).map(|p| p[1] - p[0]).collect();
                assert!(gaps.windows(2).all(|pair| pair[0] == pair[1]), "{width}: {row}");
            }
        }
    }
}
...
fn all_ranking_rows_share_column_widths_and_numeric_right_edges() {
    ...
    for width in 32..=120 {
        ...
        let widths: Vec<_> = rows[0].spans.iter().map(|span| columns(&span.content)).collect();
        for (index, row) in rows.iter().enumerate() {
            assert_eq!(row.width(), width);
            assert_eq!(row.spans.iter().map(|span| columns(&span.content)).collect::<Vec<_>>(), widths);
            if index > 0 {
                ...
                assert!(row.spans[7].content.ends_with(&compact(model.total_tokens())));
            }
        }
...
fn model_names_are_provider_free_utf8_safe_and_at_most_fifteen_bytes() {
    for name in ["provider/abcdefghijklmnopqr", "vendor/智谱大模型最新版本", "vendor/e\u{301}-long-model-name"] {
        for width in 0..=30 {
    ...
    assert_eq!(model_name("vendor/abcdefghijklmnopqr", 20), "abcdefghijklmno");
}

#[test]
fn monthly_layout_uses_two_left_charts_and_one_full_height_ranking() {
    let usage = sample_usage();
    let now = chrono::DateTime::parse_from_rfc3339("2026-09-12T12:00:00+00:00").unwrap().timestamp();
    for width in [66, 80, 100, 140, 180] {
        for monthly_height in [12, 17, 24, 35] {
            ...
            terminal.draw(|frame| super::super::draw_charts(frame, &areas, &usage, now)).unwrap();
            let buffer = terminal.backend().buffer();
            let rows: Vec<_> = (ranking.y..ranking.bottom()).filter_map(|y| {
            ...
            for (index, (_, text)) in rows.iter().enumerate() {
                assert!(text.trim_start().starts_with(&format!("{}.", index + 1)), "{text}");
...
fn small_monthly_panes_keep_full_width_charts() {
    for (width, height) in [(40, 24), (65, 24), (100, 10)] {
        let areas = [Rect::new(0, 0, width, 8), Rect::new(0, 8, width, height / 2), Rect::new(0, 8 + height / 2, width, height - height / 2)];
...
fn empty_or_short_rankings_do_not_duplicate_or_invent_models() {
    ...
    for count in [0, 1, 3] {
        ...
        terminal.draw(|frame| draw_monthly_ranking(frame, frame.area(), &usage.month_models[..count])).unwrap();
        let buffer = terminal.backend().buffer();
        let rows: Vec<_> = (0..20).map(|y| row_text(buffer, buffer.area, y)).collect();
        assert_eq!(rows.iter().filter(|row| row.contains("model-")).count(), count);
        assert_eq!(rows.iter().any(|row| row.contains("暂无本月用量记录")), count == 0);
    }
}
