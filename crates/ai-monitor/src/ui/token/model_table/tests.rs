...
fn sample_models() -> Vec<ModelUsage> {
    ...
    .into_iter()
    .map(
        |(name, input, basis_points, output, reasoning, cost)| ModelUsage {
            model: name.into(),
            input_total: input,
...
fn right_edge(row: &str, value: &str) -> usize {
    let start = row
        .find(value)
        .unwrap_or_else(|| panic!("{value} missing in {row}"));
...
fn names_have_no_provider_and_at_most_25_characters() {
    ...
    assert_eq!(
        model_name(long, 100),
        long.chars().take(25).collect::<String>()
    );
    assert_eq!(
        model_name(&format!("provider/{}", "é".repeat(30)), 100),
        "é".repeat(25)
    );
    for name in [
        "供应商/深度求索模型名称测试-very-long-model-name",
        "vendor/🦀🦀-long-model-name-that-overflows",
    ] {
        for width in 0..=100 {
...
fn display_name_collisions_do_not_merge_models_or_reorder_totals() {
    let models = vec![
        ModelUsage {
            model: "one/same-model".into(),
            input_total: 100,
            ..ModelUsage::default()
        },
        ModelUsage {
            model: "two/same-model".into(),
            input_total: 200,
            ..ModelUsage::default()
        },
    ];
    ...
    for width in [8u16, 16, 25, 46, 64, 81, 100, 160] {
        ...
        terminal
            .draw(|frame| {
                ...
                frame.render_widget(
                    Paragraph::new(model_table(&sample_models(), width as usize, 6)),
                    area,
                );
            })
            .unwrap();
...
fn export_real_model_table_terminal_fixtures() {
    ...
    let output = std::env::var_os("MODEL_PREVIEW_DIR")
        .map(PathBuf::from)
