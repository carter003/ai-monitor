...
fn every_line_is_bounded_including_unicode_errors_and_notes() {
    ...
    states[2].cards[0].note =
        Some("很长的说明\twith a newline\nthat must not break the grid".into());
    states[3].error =
        Some("网络错误：upstream timeout with an unusually long error message".into());
...
fn all_meters_share_percentage_bar_and_reset_columns() {
    ...
    for row in meters {
        ...
        assert_eq!(
            columns(&value[..percent_at]),
            cols.margin + cols.label + cols.value
        );
        assert_eq!(
            columns(&value[..bar_at]),
            cols.margin + cols.label + cols.value + 2
        );
    ...
    for width in 11..22 {
        ...
        for expected in ["96.32%", "100.00%", "5d 18h", "1d 00h", "5H", "周"] {
            assert!(
                ...
                "width {width}: missing {expected}: {output}"
            );
        }
        assert!(!output.contains('━'));
...
fn account_names_are_muted_and_truncated_before_status() {
    ...
    let name = row
        .spans
        .iter()
        .find(|s| s.content.contains("AGY2"))
        .unwrap();
    let detail = row
        .spans
        .iter()
...
fn balance_keeps_usd_cents_and_right_alignment() {
    ...
    for width in [30, 46] {
        ...
        let amount = rows[1]
            .spans
            .iter()
            .find(|s| s.content.contains("$39.83"))
            .unwrap();
...
fn low_and_stale_balances_keep_their_warning_semantics() {
    for (balance, is_stale, expected) in [
        (0., false, AMBER),
        (0.99, false, AMBER),
        (39.83, true, MUTED),
    ] {
...
fn quota_thresholds_do_not_color_the_reset_countdown() {
    for (remaining, expected) in [
        ...
        let reset = rows[0]
            .spans
            .iter()
            .find(|s| s.content.contains("1h 00m"))
            .unwrap();
...
fn unknown_available_quota_is_not_rendered_as_full() {
    ...
    assert!(
        rows[0]
            ...
            .all(|s| s.style.fg == Some(BAR_TRACK))
    );
}
