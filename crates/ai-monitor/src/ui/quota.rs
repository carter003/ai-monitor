...
impl Columns {
    fn new(width: u16, states: &[SourceState], now: i64) -> Self {
        ...
        let label = meters().map(|m| columns(&m.label)).max().unwrap_or(0).clamp(4, 8);
        ...
        Self { margin, width, label, value, reset, bar, stacked }
    }
    ...
    fn row(self, label: &str, value: &str, bar: Vec<Span<'static>>, reset: &str,
        value_style: Style) -> Line<'static> {
        ...
        if self.bar > 0 {
            ...
            let gap = self.width.saturating_sub(self.label + 1 + self.value + self.reset);
...
fn cell(text: &str, width: usize, alignment: Alignment, style: Style) -> Span<'static> {
    ...
    Span::styled(format!("{}{}{}", " ".repeat(left), text, " ".repeat(padding - left)), style)
}
...
pub(super) fn panel(frame: &mut Frame, area: Rect, states: &[SourceState], now: i64) -> Rect {
    let title = " AI 额度 · 剩余 ";
    let count = format!(" {} 项 ", states.iter().map(|s| s.cards.len()).sum::<usize>());
    ...
    if inner.height >= 2 && !cols.stacked {
        let bar = vec![cell("进度", cols.bar, Alignment::Center, Style::default().fg(MUTED))];
        let heading = cols.row("周期", "剩余", bar, "距重置", Style::default().fg(MUTED));
        frame.render_widget(Paragraph::new(heading), Rect::new(inner.x, inner.y, inner.width, 1));
        inner.y += 1;
        inner.height -= 1;
    }
    inner
...
fn stale(state: &SourceState, now: i64) -> bool {
    ...
    state.error.is_some()
        || (!held && state.fetched_at.is_some_and(|t| {
            now.saturating_sub(t).max(0) as u64 > state.refresh_interval.saturating_mul(2).as_secs()
        }))
}

fn heading(state: &SourceState, card: &Card, cols: Columns, now: i64, is_stale: bool) -> Line<'static> {
    let mut status = if state.refreshing {
        ...
        format!("{}{}前", if is_stale { "旧 " } else { "" }, age(now.saturating_sub(at)))
    } else {
        "未连接".into()
    };
    if columns(&status) + 5 > cols.width {
        status = if is_stale { "旧" }
            else if state.refreshing { "刷新" }
            else if state.hold_until.is_some_and(|t| t > now) { "等重置" }
            else if state.fetched_at.is_none() { "未连接" }
            else { "" }.into();
        if columns(&status) + 5 > cols.width {
            status.clear();
        }
    ...
    let name_width = cols.width.saturating_sub(status_width + usize::from(!status.is_empty()));
    let (name, detail) = if let Some((name, detail)) = card.title.split_once(" · ") {
        (name, format!(" · {detail}"))
    } else if let Some((name, detail)) = card.title.strip_suffix(')').and_then(|s| s.split_once(" (")) {
        (name, format!(" · {detail}"))
    } else {
    ...
    cols.line(vec![
        cell(name, primary_width, Alignment::Left, bg.fg(INK).add_modifier(Modifier::BOLD)),
        cell(&detail, name_width - primary_width, Alignment::Left, bg.fg(MUTED)),
        Span::styled(if status.is_empty() { "" } else { " " }, bg),
        cell(&status, status_width, Alignment::Right, bg.fg(if is_stale { AMBER } else { MUTED })),
    ])
}
...
fn meter_lines(meter: &Meter, cols: Columns, now: i64, is_stale: bool) -> Vec<Line<'static>> {
    ...
    let value = remaining.map(|v| percent(v, meter.decimals)).unwrap_or_else(|| "—".into());
    let reset = reset_text(meter, now);
    let color = if is_stale || meter.expired(now) { MUTED } else { quota_color(remaining) };
    ...
    if cols.stacked {
        ...
        if cols.width > columns(&meter.label) + value_width {
            rows.push(cols.line(vec![
                cell(&meter.label, cols.width - value_width, Alignment::Left, muted),
            ...
            rows.push(cols.line(vec![cell(&value, cols.width, Alignment::Right, value_style)]));
        }
        let reset = if columns(&reset) <= cols.width { reset } else { "重置…".into() };
        rows.push(cols.line(vec![cell(&reset, cols.width, Alignment::Right, muted)]));
        return rows;
    }
    let filled = remaining.map_or(0, |v| (v.clamp(0., 100.) * cols.bar as f64 / 100.).floor() as usize).min(cols.bar);
    let bar = vec![
        Span::styled("━".repeat(filled), Style::default().fg(color)),
        Span::styled("━".repeat(cols.bar - filled), Style::default().fg(BAR_TRACK)),
    ];
    vec![cols.row(&meter.label, &value, bar, &reset, value_style)]
}

fn balance_line(balance: f64, cols: Columns, is_stale: bool) -> Line<'static> {
    let color = if is_stale || !balance.is_finite() { MUTED }
        else if balance < 1. { AMBER } else { GREEN };
    let mut amount = if balance.is_finite() { format!("${balance:.2}") } else { "—".into() };
    ...
    let currency = if cols.width >= columns(&amount) + 4 { " USD" } else { "" };
    let left = cols.width.saturating_sub(columns(&amount) + columns(currency));
    let label = if left >= 9 { "可用余额" } else if left >= 5 { "余额" } else { "" };
    ...
    for state in states {
        ...
        for card in &state.cards {
            ...
            if let Some(error) = &state.error {
                lines.push(cols.line(vec![cell(error, cols.width, Alignment::Left, Style::default().fg(AMBER))]));
            } else if card.meters.is_empty() && card.balance.is_none() && card.note.is_none() {
                let message = if state.refreshing { "正在读取额度…" } else { "暂无可用额度" };
                lines.push(cols.line(vec![cell(message, cols.width, Alignment::Left, Style::default().fg(MUTED))]));
            }
            ...
            if let Some(note) = &card.note {
                lines.push(cols.line(vec![cell(note, cols.width, Alignment::Left, Style::default().fg(MUTED))]));
            }
        }
    }
    lines
